...
import (
	...
	"github.com/SongOf/edge-storage-mapper/utils"
	"html/template"
...
func (c *UserController) Login() {
	...
	if c.Ctx.Input.IsPost() {
		...
		c.ParseForm(&form)
		user := models.FindUser(form.Username)

		md5, _ := utils.Md5(user.LoginName + form.Password + user.Salt)
		if md5 == user.Password {
			c.SetSession("username", user.UserName)
			c.SetSession("loginname", user.LoginName)
